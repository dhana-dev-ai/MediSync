...
Use null for empty fields. Return ONLY JSON."""
        ...
        # Deduplicate medications - normalize and remove duplicates
        if extracted_data.get("medications"):
            ...
            for med in extracted_data["medications"]:
                if isinstance(med, str):
                    # Normalize: lowercase and remove extra spaces for comparison
                    med_normalized = " ".join(med.split()).lower()
                    
                    # Check if we've seen this medication before
                    if med_normalized not in seen_meds:
                        seen_meds.add(med_normalized)
                        unique_meds.append(med)  # Keep original (non-normalized) for display
            
            extracted_data["medications"] = unique_meds
        
        # Deduplicate secondary diagnoses if present
        if extracted_data.get("secondary_diagnoses"):
            unique_diagnoses = []
            seen_diagnoses = set()
            
            for diagnosis in extracted_data["secondary_diagnoses"]:
                if isinstance(diagnosis, str):
                    diagnosis_normalized = " ".join(diagnosis.split()).lower()
                    if diagnosis_normalized not in seen_diagnoses:
                        seen_diagnoses.add(diagnosis_normalized)
                        unique_diagnoses.append(diagnosis)
            
            extracted_data["secondary_diagnoses"] = unique_diagnoses
        
        # Deduplicate follow-ups if present
        if extracted_data.get("follow_up"):
            unique_followups = []
            seen_followups = set()
            
            for followup in extracted_data["follow_up"]:
                if isinstance(followup, str):
                    followup_normalized = " ".join(followup.split()).lower()
                    if followup_normalized not in seen_followups:
                        seen_followups.add(followup_normalized)
                        unique_followups.append(followup)
            
            extracted_data["follow_up"] = unique_followups
