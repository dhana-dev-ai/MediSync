...
class PatientKnowledgeGraph:
    ...
    def add_medication(self, medication: MedicationRecord) -> None:
        """Add a new medication (with deduplication)"""
        # Check if medication already exists (case-insensitive name and dosage match)
        for existing in self.current_medications:
            if (existing.name.lower() == medication.name.lower() and 
                existing.dosage.lower() == medication.dosage.lower()):
                # Medication already exists, skip adding duplicate
                return
