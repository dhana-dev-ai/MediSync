...
from utils_ocr_email import extract_discharge_summary_from_image, get_secure_api_key
...
def main():
    ...
    if page == "Dashboard":
        ...
        if kg.discharge_summary:
            ...
            # Health Metrics and Follow-ups Row
            ...
            with col2:
                ...
                if interactions:
                    for i, interaction in enumerate(interactions[:2], 1):
                        severity = "🔴" if interaction.severity.name == "CRITICAL" else "🟡" if interaction.severity.name == "MAJOR" else "🟢"
                        st.write(f"{severity} {interaction.drug1} + {interaction.drug2}")
            ...
            if meds:
                # Create a detailed medication table
                med_data = []
                for med in meds:
                    med_data.append({
                        "Name": med.name,
    ...
    # ========== PAGE: UPLOAD PATIENT ==========
        ...
        with tab2:
            ...
            # Check if API key is configured - use secure retrieval
            google_api_key = None
            try:
