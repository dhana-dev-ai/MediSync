...
class AnalyzerAgent:
    ...
    def _extract_medications(self, med_text: str) -> List[MedicationRecord]:
        """
        Extract medications from text with deduplication
        Supports various formats: "Name Dosage Frequency" or "Name, Dosage, Frequency"
        """
        medications = []
        seen_meds = set()  # Track (name, dosage) tuples to avoid duplicates
        ...
        for entry in med_entries:
            ...
            if len(parts) >= 1:
                name = parts[0].lower()
                dosage = parts[1].lower() if len(parts) > 1 else "as prescribed"
